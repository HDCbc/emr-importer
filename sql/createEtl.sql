...
CREATE UNLOGGED TABLE IF NOT EXISTS etl.patient_state (emr_patient_id text, state text, effective_date timestamp without time zone, emr_reference text);
ALTER TABLE etl.patient_state SET (autovacuum_enabled = false, toast.autovacuum_enabled = false);

CREATE UNLOGGED TABLE IF NOT EXISTS etl.entry_attribute (source_table text, attribute_id numeric(6,3), emr_entry_id text, code_system text, code_value text, text_value text, date_value date, boolean_value boolean, numeric_value numeric(18,6), emr_id text, effective_date timestamptz, emr_reference text);
ALTER TABLE etl.entry_attribute SET (autovacuum_enabled = false, toast.autovacuum_enabled = false);

CREATE UNLOGGED TABLE IF NOT EXISTS etl.entry_state (source_table text, entry_type_id int, emr_entry_id text, state text, effective_date timestamp without time zone, emr_reference text);
